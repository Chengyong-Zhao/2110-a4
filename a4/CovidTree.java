...
/**
 * Important notes 1. Remember that a variable declared as: CovidTree ct; can be
 * viewed in two ways. (1) It is a node of a tree (2) It is the whole tree! More
 * specifically, the root of the tree.
 *
 * At any point, we may talk about "node ct" or "tree ct" or "the tree with root
 * ct". It all depends on what is important at this point.
 *
 * 2. For this particular class and assignment, we also use the following
 * terminology. Let p be a Person. When we talk of
 *
 * . . . . "CovidTree p",
 *
 * we mean the CovidTree node that has p in field person. <br>
 * This saves a lot of verbiage; it shortens explanations and specifications.
 * <br>
 * It's OK to use this abbreviation because, in this project, no Person <br>
 * can appear in more than once in a CovidTree.
 *
 * An instance of CovidTree represents the spread of covid among a Network of
 * people. <br>
 ...
 * From there, each person in the CovidTree is the child of the person who gave
 * <br>
...
public class CovidTree {
	 ...
	 * The person who got ill first and indirectly caused everyone in it to get ill.
	 * <br>
	 * All nodes of a CovidTree have different Persons in them. There are no
	 * duplicates
	 */
	 ...
	 * Tree t and its copy have no node in common (but nodes can share a Person).
	 * <br>
	...
	public CovidTree insert(Person p, Person c) throws IllegalArgumentException {
		...
		if (!p.equals(this.node(p).person()))
			throw new IllegalArgumentException("p is not in this CovidTree");
		if (this.node(c) != null)
			throw new IllegalArgumentException("c is already in this CovidTree");

		CovidTree newNode = new CovidTree(c);
		node(p).children.add(newNode);
		return newNode;
	...
	public int size() {
		...
		if (children.size() == 0) {
			// System.out.print(this.person.toString());
			return 1;
		}
		...
		// add it self
		return sum + 1; // children.size() == 0 ? sum :
//		Iterator<CovidTree> it = children.iterator();
//		while (it.hasNext()) {
//			sum += it.next().size();
//		}
	}
	...
	public int depth(Person p) {
		...
		int child_depth = 0;
		for (CovidTree child : children) {
			child_depth = child.depth(p) + 1;
//			System.out.println(child.person().toString());
//			System.out.println(child_depth);
			if (child_depth != 0)
				return child_depth;
		}
		// return -1 offset the +1 in child_depth
		return -1;
	}
...
//		System.out.println(d);
		...
		int sum = 0;
		for (CovidTree child : children) {
//			System.out.println("in iterator: " + child.person.toString());
	...
	public List<Person> covidRouteTo(Person c) {
		...
		// use something that implements it. LinkedList<Person> is preferred to
		// ArrayList<Person>,
		...
		// case, when c is the root. Do NOT create a new list object using a
		// new-expression
		// in the recursive case.

		// 3. If c is null, there is no route to it, so null should be returned. But do
		// NOT
		...
		if (c != null && contains(c)) {
			for (CovidTree child : children) {
				if (child.contains(c)) {
					List<Person> list = child.covidRouteTo(c);
					list.add(0, person); // itself to list
					return list;
				}
			}
		}
	...
	/**
	 * If either child1 or child2 is null or is not in this CovidTree, return
	 * null.<br>
	...
	public Person commonAncestor(Person child1, Person child2) {
		// TODO 7
		/*
		 * Using getParent will give you 0 for this method. Its use over and over is
		 * inefficient.
		 *
		 * Instead, find the covid routes l1 and l2 to the two children. If both are not
		 * null, then two things are known: (1) l1[0] = l2[0] (2) the answer l1[i] is
		 * the largest i such that l1[0..i] = l2[0..i]. If this is not clear, draw an
		 * example. The answer, then, can be found using a loop. No recursion is needed.
		 *
		 * You have a problem of writing this loop efficiently. You can't use a foreach
		 * loop on both lists simultaneously. The simplest thing to do is to use List's
		 * function toArray and work with the array representations of the lists.
		 */
		if (covidRouteTo(child1) != null && covidRouteTo(child2) != null) {
			Object[] pl1 = covidRouteTo(child1).toArray();
			Object[] pl2 = covidRouteTo(child2).toArray();
			for (int i = Math.min(pl1.length - 1, pl2.length - 1); i >= 0; i--) {
				if (pl1[i].equals(pl2[i])) // largest i
					return (Person) pl1[i];
			}
		}
		return null;
	...
	public boolean equals(Object ob) {
		// TODO 8
		// The specification outlines what must be done in detail. Two points to
		// discuss:
		// (1) Stay away from nested ifs as much as possible! Instead, as soon as it is
		// determined that a property for equality is not met (or is met), return false
		// (or true)!
		...
		// satisfy 1. and 2.(1) (2)
		if (ob == null || getClass() != ob.getClass() || person != ((CovidTree) ob).person()
				|| childrenSize() != ((CovidTree) ob).childrenSize())
			return false;
		if (childrenSize() == 0)
			return true;

		// satisfy 2.(2) (3)
		Set<CovidTree> ob_children = ((CovidTree) ob).copyOfChildren();
		boolean flag = true;
		for (CovidTree child : children) { // pairwise match
			if (!help(child, ob_children)) {
				flag = false;
			}
		}
		return flag || false;
	...
	private boolean help(CovidTree t1, Set<CovidTree> s2) {
		boolean flag = false;
		for (CovidTree ob_child : s2) {
//			System.out.println("flag: " + flag);
			flag = t1.equals(ob_child);
//			System.out.println("flag: " + flag);
			if (flag) {
//				System.out.println("true");
				if (t1.childrenSize() != ob_child.childrenSize() || depth(t1.person) != depth(ob_child.person)) {
					flag = false;
				}
//				System.out.println("keep true");
				break;
			}
		}
		return flag;
	 ...
	 * GUI and in the grading program. Feel free to read/study them.
	 */
	...
	/**
	 * For each node of this CovidTree, which is at some depth d in this
	 * CovidTree,<br>
	 ...
	 * If this CovidTree has no children (it is a leaf), return the root's
	 * substring.<br>
	 * Otherwise, return<br>
